...
#    sync_files(local_directory)
            ...
            # Check if file is in database (ASSESSMENT PHASE LOGIC)
            if self._is_file_in_database_single(original_size, filetype):
                print(f"[DEBUG] Single process - File found in database: {file_path_obj.name}")
                
                # Get resized size from database
                resized_size = self.database[db_key]['resized_size']
                result_data['resized_size'] = resized_size
                
                # Check if already synced with Cloudinary
                if self._is_file_synced_single(file_path_obj, resized_size):
                    print(f"[DEBUG] Single process - File already synced: {file_path_obj.name}")
                    result_data['already_synced'] = True
                    return True, result_data, None
                else:
                    print(f"[DEBUG] Single process - File in DB but not synced, will resize and upload: {file_path_obj.name}")
                    # File needs to be resized and uploaded
                    optimized_file = self._resize_and_upload_single(file_path_obj, result_data, db_key)
                    return True, result_data, optimized_file
            else:
                print(f"[DEBUG] Single process - File not in database, processing: {file_path_obj.name}")
                # File not in database - resize now and add to database
                optimized_file = self._process_new_file_single(file_path_obj, result_data, db_key)
                return True, result_data, optimized_file
                
        except Exception as e:
            print(f"[ERROR] Single process failed for {file_path}: {e}")
    ...
    def _load_database_and_cloudinary_data(self, settings_dialog=None):
        ...
        try:
            ...
            self.cloudinary_files = list_all_files(self)
    ...
    def _resize_and_upload_single(self, file_path_obj, result_data, db_key):
        """Resize and upload a file that's already in the database"""
        try:
            # Create temp directory for this single file
            temp_dir = Path(tempfile.mkdtemp(prefix="happytag_single_"))
            ...
            if hasattr(self, 'cloudName') and self.cloudName:
                try:
                    # Get folder name for upload
                    if hasattr(self, 'original_folder_name') and self.original_folder_name:
                        folder_name = f"{CLOUDINARY_FOLDER_PREFIX}/{self.original_folder_name}"
                    else:
                        folder_name = CLOUDINARY_FOLDER_PREFIX
                    ...
                    print(f"[WARNING] Single process - Upload error for {file_path_obj.name}: {e}")
            
            return str(resized_file_path)
            
        except Exception as e:
            print(f"[ERROR] Single process - Resize/upload failed for {file_path_obj.name}: {e}")
            return None
    
    def _process_new_file_single(self, file_path_obj, result_data, db_key):
        """Process a new file that's not in the database"""
        try:
            # Create temp directory for this single file
            temp_dir = Path(tempfile.mkdtemp(prefix="happytag_single_"))
            ...
            # Check if already synced after getting resized size
            if not self._is_file_synced_single(file_path_obj, resized_size):
                ...
                if hasattr(self, 'cloudName') and self.cloudName:
                    try:
                        ...
                        print(f"[WARNING] Single process - Upload error for new file {file_path_obj.name}: {e}")
            else:
                result_data['already_synced'] = True
                print(f"[DEBUG] Single process - New file already synced: {file_path_obj.name}")
            ...
            print(f"[ERROR] Single process - Processing failed for new file {file_path_obj.name}: {e}")
            return None
